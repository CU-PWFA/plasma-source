...
# -*- coding: utf-8 -*-
"""
Created on Wed May 23 16:47:38 2018
...
if __name__ == '__main__':
    ...
    dgb    = 0.0025    # relative energy spread (HWHM)
    dz     = 0       # spread in z (HWHM)
    eps    = 3.2e-6  # m-rad, normalized emittance
    beta   = 0.10    # m, Twiss at vac. waist
    alpha  = 0.00    # Twiss at vac. waist
    gamma  = (1.0+alpha**2)/beta # 1/m, Twiss at vac. waist
    auto_match = False #True # auto-match beam to plasma flat-top
    ...
    L_up     = 0.75 # m, full length of ramp
    ...
    L_dn     = 0 #L_up     # m, full length of ramp
    top_dn   = 0        # m, relative location of ramp top
    
    # calculate betatron wave number in flat-top
    wpft    = (5.64e4)*np.sqrt(npl0) # rad/s, plasma ang. freq. (flat-top)
    kpft    = wpft/nc.c               # 1/m, plasma wave number (flat-top)
    kbft    = kpft/np.sqrt(2*gbC)     # 1/m, betatron wave number (flat-top)
    
    # define longitudinal steps
    ds   = (1.0/kbft)*(1./10.)                 # m, step size
...
#    ds   = beta/10.
    ...
    # calculate initial betatron wave number
    wp0    = (5.64e4)*np.sqrt(plasma["npl"][0]) # rad/s, plasma ang. freq. (flat-top)
    ...
    if (auto_match):
        beta   = 1.0/kb0
        dndz0 = (plasma["npl"][1]-plasma["npl"][0])/ds
        alpha  = -dndz0/(4*plasma["npl"][0]*kb0) #0.00    # Twiss at vac. waist
        gamma  = (1.0+alpha**2)/beta # 1/m, Twiss at vac. waist
    ...
    x_offset = 3.1*(102e-6) #1.92*np.sqrt(kp0/kpft)/kp0
    print('sigx: ',sigx)
    print('kb-1: ',1/kb0)
    print('kp-1: ',1/kp0)
    print('x offset: ',x_offset)
#    pb.add_offset(ebeam,'x',x_offset)
    pb.add_offset(ebeam,'xp',2.5e-3)
