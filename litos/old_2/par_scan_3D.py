...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 27 17:14:02 2017
...
if __name__ == '__main__':
    ...
    # specify plasma density scan values
    nnpl0 = 10 # number of values
    npl0  = np.linspace(1e16,1e17,nnpl0) # cm^-3, flat-top plasma density
    
    for inpl0 in npl0:
        print(inpl0)
        
        # define plasma bulk (flat-top) properties
        dEds0  = np.sqrt(inpl0/(1e17))*10e9 # eV/m, energy gain rate
        ...
        wp0    = (5.64e4)*np.sqrt(inpl0) # rad/s, plasma ang. freq. (flat-top)
        ...
        # make plasma
        bulk    = ps.make_bulk(s_ft,inpl0,dgds0)
        up_ramp = ps.make_ramp(s_up,'up',shape_up,hw_up,top_up,inpl0,dgds0)
        dn_ramp = ps.make_ramp(s_dn,'dn',shape_dn,hw_dn,top_dn,inpl0,dgds0)
        plasma0 = ps.make_plasma(bulk,up_ramp,dn_ramp) # output: plasma dict.

        # specify waist scan values
        nwaist = 35 # number of values
        waist  = np.linspace(-0.45,-0.28,nwaist) # m, waist location w.r.t. L_up
    
        # specify ramp half-width scan values
        nhw_up = 21 # number of values
