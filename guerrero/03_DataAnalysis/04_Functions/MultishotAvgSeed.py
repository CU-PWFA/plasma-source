...
#    os.chdir(os.path.expanduser(DataDir))
    ...
    for name in glob.glob(DataDir+ '17529184_??????????_????.tiff'):
        DataNumber= name[-20:-10]
    print(DataNumber)
    ...
    for shot in range (0, shotN):
        ...
        #load the tiff image and change it to array
            data= np.array(Image.open(DataDir+ FileName))
...
#        plt.plot(Peak, np.sum(-signalFiltered, axis= 1)[Peak], 'x')
    ...
    try:
        ...
        np.save(os.path.join(ResultDir, '01_npy', DataNumber+'_NorSignal'), \
                (signalFiltered-np.amin(signalFiltered))/np.amax(signalFiltered-np.amin(signalFiltered)))
        np.save(os.path.join(ResultDir, '01_npy', DataNumber+'_Signal'), signal)
        np.save(os.path.join(ResultDir, '01_npy', DataNumber+'_Data'), AvgData)
        np.save(os.path.join(ResultDir, '01_npy', DataNumber+'_BG'), AvgBG)
        ...
        plt.savefig(ResultDir+'/02_Plots/'+DataNumber+'_NorSignal')
        plt.close()
        ...
        plt.savefig(ResultDir+'/02_Plots/'+DataNumber+'_Signal')
        plt.close()
        
        plt.figure()
        plt.savefig(ResultDir+'/02_Plots/'+DataNumber+'_ManualCheck')
        plt.close()
