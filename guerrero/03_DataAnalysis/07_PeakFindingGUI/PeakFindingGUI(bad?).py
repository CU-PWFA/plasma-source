...
#decide if that is ok
 ...
 #click the peak
 #let it run 
os.chdir(os.path.expanduser('~/Desktop/Guerrero DAQ local copy/plasma-source-master/lee/03_DataAnalysis/04_Functions/'))
from DataToSignal import DataToSignal
from MultishotAvgSeed import MultishotAvgSeed

os.chdir(os.path.expanduser('~/Desktop/Guerrero DAQ local copy/plasma-source-master/lee/03_DataAnalysis/03_LineFindingGUI/'))
...
def SavePeak(event):
    ...
    if FirstPeaks1.size!= 0 and FirstPeaks2.size!= 0:
        ...
        np.save(os.path.join(os.path.expanduser('~/Desktop/Guerrero DAQ local copy/plasma-source-master/lee/03_DataAnalysis/05_ParameterScan/01_Ar/01_PressureScan/03_PeaksFoundByClickyTool/'), DataNumber+ '_PeaksInfo.npy'), \
                CombinedResult)
...
#    os.chdir(os.path.expanduser('~/Dropbox/01_Research/01_Project/01_PlasmaDianostics/01_Shadowgraphy/03_DataAnalysis/05_ParameterScan/01_Ar/01_PressureScan/02_MultishotsAvgResults/01_npy'))
    for name in glob.glob(os.path.expanduser('~/Desktop/Guerrero DAQ local copy/plasma-source-master/lee/03_DataAnalysis/05_ParameterScan/01_Ar/01_PressureScan/02_MultishotsAvgResults/01_npy/')+'??????????_NorSignal.npy'):
        DataNumber= name[157:167]
        #print(DataNumber)
        if (DataNumber not in SkipList):
            print(name)
            SignalArray= np.load(name)
            img= ImageTk.PhotoImage(image= Image.fromarray(np.uint8(cm.inferno(SignalArray)*255)))
            LO= ImageTk.PhotoImage(Image.open(os.path.expanduser('~/Desktop/Guerrero DAQ local copy/plasma-source-master/lee/03_DataAnalysis/05_ParameterScan/01_Ar/01_PressureScan/02_MultishotsAvgResults/02_Plots/'+ \
...
def LoadRawSignal(event):
    ...
    global MediaPath
    global avgBG
    avgBG= np.load(os.path.expanduser('~/Desktop/Guerrero DAQ local copy/plasma-source-master/lee/03_DataAnalysis/05_ParameterScan/01_Ar/01_PressureScan/02_MultishotsAvgResults/01_npy/')+ DataNumber+ '_BG.npy')
    MediaPath= '~/Desktop/Guerrero DAQ local copy/DAQ_Data/mnt/md0/DAQ/IMAGE/year_2021/month_10/day_29/'
...
#show the one shot and the sum lineout    
    SF= ImageTk.PhotoImage(image= Image.fromarray(np.uint8(cm.inferno((SignalFiltered-np.amin(SignalFiltered))/np.amax(SignalFiltered-np.amin(SignalFiltered)))*255)))
...
def ReMatch(event):
    ...
    ResultDir= '~/Desktop/Guerrero DAQ local copy/plasma-source-master/lee/03_DataAnalysis/05_ParameterScan/01_Ar/01_PressureScan/02_MultishotsAvgResults/'
    seed= y1
