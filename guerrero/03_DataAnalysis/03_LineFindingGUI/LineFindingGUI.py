...
def LoadSignal(event):
    ...
    os.chdir(os.path.expanduser('~/Desktop/Guerrero DAQ local copy/plasma-source-master/lee/03_DataAnalysis/05_ParameterScan/01_Ar/01_PressureScan/02_MultishotsAvgResults/01_npy'))
    for name in glob.glob('??????????_NorSignal.npy'):
        DataNumber= name[0:10]
        CheckRun= os.path.expanduser('~/Desktop/Guerrero DAQ local copy/plasma-source-master/lee/03_DataAnalysis/05_ParameterScan/01_Ar/01_PressureScan/03_PeaksFoundByClickyTool/')+ \
