...
plasmaParams = {
    ...
    'Nz' : 2**10,
    'X' : 0.14e6,#0.060e6,
    'Y' : 3000,#6000, <- it works when 6000
    'Z' : 1.5e6, 
...
#%%
propE= laser.fourier_prop2(beam.e, xgridP, ygrid, 0.3e6, 0.8)
#%%
plt.figure(3)
#plt.axis('scaled')
...
imaging=phaselens.phase_lens_2d(Physical_X, Physical_Y, E_afterPlasma, 0.5e6, 1.4615e6, 0.646e6, beamParams['lam'])
...
np.save('PW'+str(PlasmaWidthS)+'_GP'+str(power)+'_n'+str(int(density*10)), LONew)
