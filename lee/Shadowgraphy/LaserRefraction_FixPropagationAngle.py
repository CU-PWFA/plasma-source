...
neNew= np.zeros((plasmaParams['Nx'], plasmaParams['Ny'], plasmaParams['Nz']))
...
halflayers= round(10*w/(plasmaParams['Y']/plasmaParams['Ny']))+1
...
for ny in range (int(plasmaParams['Ny']/2-halflayers), int(plasmaParams['Ny']/2+halflayers)):
    ...
    neNew[:, ny, int(Zdiff/2):int(testGrid2.shape[1]+Zdiff/2)]= neTemp[int(Xdiff/2):int(Xdiff/2+plasmaParams['Nx']), :]
