...
w = PlasmaWidthS
z0 = 0.85e6 #0.6e6 #The distance at which the uniform fully ionized plasma starts.
zf = plasmaParams['Z']
...
#%%
theta = 0.859 # degrees
factor= (plasmaParams['Z']/plasmaParams['Nz'])/(plasmaParams['X']/plasmaParams['Nx'])
nx= np.linspace(0, plasmaParams['Nx']-1, plasmaParams['Nx'])
nz= np.linspace(0, plasmaParams['Nz']-1, plasmaParams['Nz'])
newnz= np.linspace(0, plasmaParams['Nz']-1, int(round(plasmaParams['Nz']*factor)))

f = interp2d(nz, nx, ne[:, 0, :], kind='linear')
neEq= f(newnz, nx)
testGrid= rotate(neEq, theta)
del f

tempz= np.linspace(0, int(testGrid.shape[1])-1, int(testGrid.shape[1]))
tempx= np.linspace(0, int(testGrid.shape[0])-1, int(testGrid.shape[0]))
tempzNew= np.linspace(0, int(testGrid.shape[1])-1, int(testGrid.shape[1]/factor))

q= interp2d(tempz, tempx, testGrid)
testGrid2= q(tempzNew, tempx)
Xdiff= abs(testGrid2.shape[0]-plasmaParams['Nx'])
Zdiff= abs(testGrid2.shape[1]-plasmaParams['Nz'])
halflayers= round(5*w/(plasmaParams['Y']/plasmaParams['Ny']))+1
Z0=testGrid2.shape[1]
del q
del testGrid
del testGrid2

for ny in range (int(plasmaParams['Ny']/2-halflayers), int(plasmaParams['Ny']/2+halflayers)):
    f = interp2d(nz, nx, ne[:, ny, :], kind='linear')
    neEq= f(newnz, nx)
    neData= rotate(neEq, theta)
    q= interp2d(tempz, tempx, neData)
    neTemp= q(tempzNew, tempx)
    ne[:, ny, int(Zdiff/2):int(Z0+Zdiff/2)]= neTemp[int(Xdiff/2):int(Xdiff/2+plasmaParams['Nx']), :]
#%%
He.initialize_plasma(n, ne)
#He.initialize_plasma(n, neNew)
del n
#del ne

#%%
plt.figure(2)
#plt.pcolormesh(neTemp)
#plt.pcolormesh(ne[:, :, 128])
plt.pcolormesh(ne[:, 255, :])
plt.colorbar()
#plt.axis('scaled')
#%%
...
beamParams = {
    ...
    'waist' :  4000, #8000, #16000,#The spot size of the flattop region.
    'z0' : 0.0,
    'order' : 3,
    'theta' : 0,#2.06175769, #The angle of propagation in degrees, positive is angled upward.
    'dx' : 0
}
