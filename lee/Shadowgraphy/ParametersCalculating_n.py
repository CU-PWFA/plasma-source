...
PeakSpacingDen= np.zeros(DensityL)
PeakSpacing2Den= np.zeros(DensityL)
PeakHRatioDen= np.zeros(DensityL)


#for density in range (1, 16, 2):
...
#        LONew= np.load('PW'+str(PlasmaWidthS)+'_GP'+str(power)+'_n'+str(density)+'.npy')
...
#            PeakSpacingDen[int((density+1)/2-1)]= PeakSpacing
#            PeakSpacing2Den[int((density+1)/2-1)]= PeakSpacing2
#            PeakHRatioDen[int((density+1)/2-1)]= PeakHRatio
