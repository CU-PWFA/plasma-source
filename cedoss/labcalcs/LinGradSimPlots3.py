...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 21 10:44:35 2023

Copy in 2023 to test fitting on a log scale
...
#Smoothed Version
dndys =          np.array([2.40e18, 5.85e17, 7.42e16, 1.63e15, 1.60e19, 7.93e18, 1.17e18])
dndys_std =      np.array([2.63e17, 6.79e16, 2.05e16, 1.88e15, 2.69e18, 1.37e18, 1.43e17])
ering_norm =     np.array([1.38e17, 3.62e16, 4.43e15, 3.83e14, 7.45e17, 3.76e17, 6.25e16])
ering_norm_std = np.array([1.26e16, 3.24e15, 4.60e14, 1.48e14, 7.42e16, 3.87e16, 1.03e16])
...
"""
"""
test = np.polyfit(np.log(dndy), np.log(dndys), 1)
arrtest = np.linspace(35,43,50)
plt.scatter(np.log(dndy),np.log(dndys))
plt.plot(arrtest,arrtest*test[0]+test[1])
plt.show()

plt.scatter(dndy,dndys)
plt.plot(np.exp(arrtest),np.power(np.exp(arrtest),test[0])*np.exp(test[1]))
#plt.plot(np.exp(arrtest),np.exp(arrtest*test[0]+test[1]))
plt.show()
...
if doLoop:
    ...
    facB_arr = np.linspace(3.1,3.2,1000)
    fitB_arr = np.zeros(len(facB_arr))
    
for i in range(len(facB_arr)):
    if doLoop:
        empB = facB_arr[i]
        #sheath_fit = np.polyfit(dndy*empB, dndys, 1, w=1/np.power(np.log(dndys_std),weightpower)*1/np.power(np.log(dndys),weightpower))
        sheath_fit = np.polyfit(dndy*empB, dndys, 1, w=1/np.power(dndys_std,weightpower)*1/np.power(dndys,weightpower))
...
if doLoop:
    ...
    empB=facB_arr[np.argmin(fitB_arr)]
    #empB = 2.9
...
#empB = 2.786
if doLoop:
    #facD_arr = np.linspace(0.085,0.095,100)
    facD_arr = np.linspace(0.090,0.091,1000)
...
for i in range(len(facD_arr)):
    if doLoop:
        ...
        #ering_fit = np.polyfit(ering_norm, ering_norm_righthand, 1, w=1/np.log(ering_norm_std))
        ering_fit = np.polyfit(ering_norm, ering_norm_righthand, 1, w=1/np.power(ering_norm_std,weightpower))
