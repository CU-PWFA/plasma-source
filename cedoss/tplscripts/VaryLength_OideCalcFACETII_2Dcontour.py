...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 24 13:43:17 2018
...
beta_i_arr = np.linspace(10, 50, 10) #cm
n0_arr = np.linspace(0.5e18, 1e18, 10) #cm^-3
...
if func == 1:
    ...
    plt.ylabel(y_label)
    plt.xlabel(x_label)
    plt.plot(beta_i_arr, nmax_arr, 'k-')
    plt.ylim((n0_arr[0],n0_arr[-1]))
    plt.show()
    ...
    plt.plot(beta_i_arr, nmax_arr, 'k-')
    plt.ylim((n0_arr[0],n0_arr[-1]))
    plt.xlim((beta_i_arr[0],beta_i_arr[-1]))
    plt.show()
    """
    ##Sigy
    X = np.tile(x_arr.reshape(-1,1),(1,len(y_arr)))
