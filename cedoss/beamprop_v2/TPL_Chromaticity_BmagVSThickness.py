...
plt.plot(len_arr*1e-6*np.sqrt(k), bmag_w2_arr, 'g-.', label = "Thin Regime",linewidth = lwid)
plt.plot(len_arr*1e-6*np.sqrt(k), bmag_w2_arr_thick, 'b--', label = "Thick Regime",linewidth = lwid)
...
sigmarp = beam.get_sigmarp(m-1)[0]
gam=19569.5
dist = Foc.Calc_ThickWaistPos_DeltaOff_UnNormalized(k,len_arr[-1],betastar,0)
emit_geo = emit_arr[-1]*emit_n/gam

emit_migliorati = np.sqrt(gam**2 * (dist**2 * (sigmaE)**2 * sigmarp**4 + emit_geo**2))
emit_m_frac = emit_migliorati/emit_n

##Nice plot of sigma r vs sqrt(k)l
plt.figure(figsize=(7,4))
#plt.title("Beam spot size vs lens thickness")
plt.semilogy(len_arr*1e-6*np.sqrt(k), sigm_arr*1e6, 'r-', label="Beam Propagation", linewidth = lwid)
