...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 21 10:00:46 2018

Chromatic analysis of TPLs with a Tanh profile
...
gammab = 30                   #PProp.def_gamma

tpl_f = 0.017                 #0.127/10
tpl_n = 4.9e12/1e17           #0.5*10
fit = [10*1e3, 4*1e3, tpl_n]  #[86.845, 19.887, tpl_n]
betastar = 0.136              #.10 #0.00213065326633
...
e_spec = np.array([0, -0.0125, 0.0125]) + 1.0
...
argon = PProp.NoPlasma_ThinPlasmaLens_Tanh(argon_params, n_arr, tpl_offset*1e6 + position_error, fit, debug)

fig, ax1 = plt.subplots()
plt.title("TANH: Beta function evolution at "+r'$f=$'+'{:.3f}'.format(tpl_f*100)+r'$\,\mathrm{cm}$')
...
ax1.plot((z_arr-leftext*tpl_f)*1e2, np.array(beta0)*1e2, 'b--',label=r'$\beta_{vac}$')
...
#Make beam and bulk plasma just as in single_pass
    ...
    ax1.plot((z_arr-2*tpl_f)*1e2, np.array(beta)*1e2, colors[i], label=r'$\gamma/\gamma_{b}$' + " = "+str(e_spec[i]))
    
ax2 = ax1.twinx()
ax2.plot((z_arr-leftext*tpl_f)*1e2, n_arr, 'k-')
...
emit = 100e-6
fig, ax1 = plt.subplots()
plt.title("TANH: Beam FWHM evolution at "+r'$f=$'+'{:.3f}'.format(tpl_f*100)+r'$\,\mathrm{cm}$')
ax1.set_ylabel(r'$FWHM\,\mathrm{[\mu m]}$', color = 'r')
ax1.tick_params('y', colors = 'r')
ax1.set_xlabel('z [cm]')
ax1.set_ylim([-0.05,2000.05])
beta0 = np.array(PProp.Calc_CSParams(beam_params0, np.zeros(len(z_arr)), z_arr)[0])
sig0 = np.sqrt(emit*beta0/gammab)
ax1.plot((z_arr-leftext*tpl_f)*1e2, 2.35*sig0*1e6, 'r--',label=r'$\sigma_{vac}$')
...
#Make beam and bulk plasma just as in single_pass
    beam_params = PProp.ReturnDefaultElectronParams(path, beta_star=betastar,
                                                   beta_offset=waist_loc, plasma_start=z_offset,
                                                   gamma=gammab * e_spec[i])
    beta = np.array(PProp.Calc_CSParams(beam_params, n_arr, z_arr)[0])
    sig = np.sqrt(emit*beta/gammab)
    ax1.plot((z_arr-2*tpl_f)*1e2, 2.35*sig*1e6, colors[i], label=r'$\gamma/\gamma_{b}$' + " = "+str(e_spec[i]))
