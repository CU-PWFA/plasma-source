...
def get_current(ind):
    '''
    Reads in the current profile provided by Claudios simulations
    ...
    z    : array_like
           Longitudinal distance corresponding to I_kA in m
    dz   : float
           Step size of z
    dt   : float
           Step size of t
    ...
    dt    = (dz * 1e-6) / c;
    # Center drive on 0
    peaks = get_peaks(I_ka);
    ti = ti - ti[peaks[0]];
    return I_ka, ti, z, dz, dt;
...
def split_I(I, ti, t_split):
    '''
    Function to split the beam current into it's contributions from the 
    drive beam and the witness beam
    Parameters:
    -----------
    I : array_like
        The beam current (kA)
    ...
    --------
    I_drive : array_like
              Current of drive bunch (same length as E)
    I_wit   : array_like  
              Current of witness bunch (same length as E)
    '''
    I_drive = np.zeros(len(I))
    I_wit   = np.zeros(len(I))
    ind     = np.argmin(abs(ti-t_split))
    I_drive[0:ind] = I[0:ind]
    I_wit[ind:-1]  = I[ind:-1]

    return I_drive, I_wit
